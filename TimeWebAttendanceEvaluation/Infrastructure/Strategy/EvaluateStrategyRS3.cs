...
{
    public class EvaluateStrategyRS3 : IEvaluateStrategy
    {
        private readonly IAttendanceRepository repo;

        public EvaluateStrategyRS3(IAttendanceRepository repo)
        {
            this.repo = repo;
        }
        ...
        public bool ExistEvent(List<DateTime> events, DateTime day)
        {
            ...
            int count = events.Where(x => 
            (x >= start.AddMinutes(-10) && x <= start.AddMinutes(10)) || 
            (x >= end.AddMinutes(-10) && x <= end.AddMinutes(10))).Count();
            if (count >= 2)
            {
                return true;
            }
